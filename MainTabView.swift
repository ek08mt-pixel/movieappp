...
struct MiniPlayerView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Button {
                withAnimation(.spring(response: 0.5, dampingFraction: 0.7)) { showYouTube.toggle() }
            } label: {
                HStack(spacing: 10) {
                    if let poster = ostManager.currentPoster, let url = URL(string: "https://image.tmdb.org/t/p/w200\(poster)") {
                        CachedAsyncImage(url: url).aspectRatio(2/3, contentMode: .fill).frame(width: 32, height: 48).clipShape(RoundedRectangle(cornerRadius: 6))
                    } else {
                        RoundedRectangle(cornerRadius: 6).fill(.ultraThinMaterial.opacity(0.5)).frame(width: 32, height: 48)
                            .overlay(Image(systemName: "music.note").font(.system(size: 12)).foregroundColor(.white.opacity(0.6)))
                    }
                    VStack(alignment: .leading, spacing: 2) {
                        Text(ostManager.currentTrack).font(.system(size: 12, weight: .semibold)).foregroundColor(.white).lineLimit(1)
                    ...
                    Button { ostManager.togglePlayback?() } label: {
                        Image(systemName: ostManager.isPlaying ? "pause.fill" : "play.fill").font(.system(size: 12)).foregroundColor(.white)
                            .frame(width: 28, height: 28).background(Circle().fill(.ultraThinMaterial.opacity(0.5)))
                    }
                    Button {
                        ...
                        Image(systemName: "xmark").font(.system(size: 8, weight: .bold)).foregroundColor(.white.opacity(0.6))
                            .frame(width: 18, height: 18).background(Circle().fill(.ultraThinMaterial.opacity(0.3)))
                    }
                }
                .padding(.horizontal, 12).padding(.vertical, 8)
                .frame(width: showYouTube ? UIScreen.main.bounds.width - 32 : 300, height: showYouTube ? 220 : 60)
                .background(RoundedRectangle(cornerRadius: showYouTube ? 20 : 30).fill(.black.opacity(0.95)).overlay(RoundedRectangle(cornerRadius: showYouTube ? 20 : 30).fill(.ultraThinMaterial.opacity(0.2))))
                .overlay(RoundedRectangle(cornerRadius: showYouTube ? 20 : 30).stroke(LinearGradient(colors: [.white.opacity(0.2), .white.opacity(0.05), .clear, .white.opacity(0.1)], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 0.8))
                .shadow(color: .black.opacity(0.5), radius: 10, y: 5)
            }.buttonStyle(.plain)
            
            if showYouTube, let ytID = ostManager.currentYoutubeID {
                YouTubeMiniWebView(youtubeID: ytID).frame(width: UIScreen.main.bounds.width - 48, height: 140).clipShape(RoundedRectangle(cornerRadius: 12)).padding(.top, 6)
            }
        }
    }
}
...
struct YouTubeMiniWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        let wv = WKWebView(frame: .zero, configuration: config); wv.backgroundColor = .black; wv.isOpaque = false; wv.scrollView.isScrollEnabled = false
        let html = """
        ...
        <script>var player;function onYouTubeIframeAPIReady(){player=new YT.Player('player',{videoId:'\(youtubeID)',width:'100%',height:'100%',playerVars:{autoplay:1,controls:0,modestbranding:1,playsinline:1},events:{onReady:function(e){e.target.playVideo()}}})}</script></body></html>
        """

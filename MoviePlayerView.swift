...
struct MoviePlayerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if showVolumeSlider {
                HStack { Spacer(); TinySlider(value: CGFloat(volume), icon: volume == 0 ? "speaker.slash.fill" : "speaker.wave.1.fill").padding(.trailing, 14) }
            }
            if showBrightnessSlider {
                HStack { TinySlider(value: brightness, icon: "sun.max.fill").padding(.leading, 14); Spacer() }
            }
            ...
            if let err = errorMessage, !isLoading {
                VStack(spacing:14) { Image(systemName:"wifi.slash").font(.system(size:36)).foregroundColor(.gray); Text(err).font(.caption).foregroundColor(.gray)
                    HStack(spacing:8) { ForEach(MovieSource.allCases, id:\.self) { s in Button { selectedSource=s; loadStream() } label: { Text(s.rawValue).font(.caption2).foregroundColor(selectedSource==s ? .white : .gray).padding(.horizontal,10).padding(.vertical,6).background(Capsule().fill(selectedSource==s ? AnyShapeStyle(.ultraThinMaterial) : AnyShapeStyle(Color.clear))) } } }
            ...
            if showSourceMenu {
                Color.black.opacity(0.01).ignoresSafeArea().onTapGesture { showSourceMenu=false }
                VStack(spacing:8) {

            ...
            VStack(spacing: 12) {
                HStack {
                    ...
                    if watchService.isInRoom {
                        Text("#\(watchService.currentRoomCode)").font(.caption).foregroundColor(.gray)
                    }
                    Spacer()
                ...
                if watchService.isInRoom {
                    ...
                    Button {
                        if !watchUserName.isEmpty {
                            watchService.createRoom(userName: watchUserName) { _ in }
                        }
                    } label: {
                        Text("Tạo phòng mới").font(.headline).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Capsule().fill(.ultraThinMaterial)).overlay(Capsule().stroke(Color.white.opacity(0.2), lineWidth: 0.5))
                    ...
                    Button {
                        if !watchUserName.isEmpty && !joinCode.isEmpty {
                            watchService.joinRoom(code: joinCode, userName: watchUserName) { success in
                                if !success { joinError = "Mã phòng không đúng" } else { joinError = "" }
                            }
                        }
                    } label: {
                        Text("Vào phòng").font(.headline).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Capsule().fill(.ultraThinMaterial)).overlay(Capsule().stroke(Color.white.opacity(0.2), lineWidth: 0.5))
                    }.padding(.horizontal, 20)
                }
            }
            .padding(20)
            .background(RoundedRectangle(cornerRadius: 24).fill(.ultraThinMaterial.opacity(0.95))).overlay(RoundedRectangle(cornerRadius: 24).stroke(.white.opacity(0.2), lineWidth: 1))

...
struct MoviePlayerView: View {
            ...
            if showControls && errorMessage == nil && !isLoading && !showOverlay && !showSourceMenu && !showSettings && !showSubtitlePopup && !showAudioPopup {
                if !isCasting {
                    HStack(spacing:64){
                        ...
                        Button{player.rate==0 ? player.play():player.pause()}label:{Image(systemName:player.rate==0 ? "play.fill":"pause.fill").font(.system(size:36,weight:.bold)).foregroundColor(.white)}
                ...
                VStack{Spacer();
                    VStack(spacing:0){
                        VStack(spacing: 6) {
                            Slider(value:$currentTime,in:0...max(duration,1)){e in isSeeking=e; if !e{player.seek(to:CMTime(seconds:currentTime,preferredTimescale:600))}}.accentColor(.white)
                            HStack{
                                Text(formatTime(currentTime)).font(.system(size:10,design:.monospaced)).foregroundColor(.white.opacity(0.7))
                                Spacer()
                                Text(formatTime(duration)).font(.system(size:10,design:.monospaced)).foregroundColor(.white.opacity(0.7))
                            }
                        }
    ...
    // MARK: - Chất lượng
    func detectQuality(from url: URL) -> String {
        let urlStr = url.absoluteString.lowercased()
        if urlStr.contains("4k") || urlStr.contains("2160") { return "4K" }
        if urlStr.contains("2880") { return "2880p" }
        if urlStr.contains("1440") { return "1440p" }
        if urlStr.contains("1080") { return "1080p" }
        if urlStr.contains("720") { return "720p" }
        if urlStr.contains("480") { return "480p" }
        return "Auto"
    }
    
    func qualityURL(from originalURL: URL, quality: String) -> URL {
        guard quality != "Auto" else { return originalURL }
        let urlStr = originalURL.absoluteString
        let qualityMap: [String: String] = [
            "4K": "2160", "2880p": "2880", "2160p": "2160",
            "1440p": "1440", "1080p": "1080", "720p": "720", "480p": "480"
        ]
        guard let targetRes = qualityMap[quality] else { return originalURL }
        
        for q in ["2160", "2880", "1440", "1080", "720", "480", "4k"] {
            if urlStr.contains(q) {
                return URL(string: urlStr.replacingOccurrences(of: q, with: targetRes, options: .caseInsensitive)) ?? originalURL
            }
        }
        
        if urlStr.contains("master") || urlStr.contains("playlist") {
            let separator = urlStr.contains("?") ? "&" : "?"
            return URL(string: "\(urlStr)\(separator)quality=\(targetRes)") ?? originalURL
        }
        return originalURL
    }
    ...
    func applyQuality(_ quality: String) {
        ...
        let newURL = qualityURL(from: url, quality: quality)
    ...
    Picker("Server", selection: $selectedServerIndex) {
        ForEach(0..<phimapiServers.count, id: \.self) { i in Text(phimapiServers[i]).tag(i) }
    }
...
ForEach(MovieSource.allCases,id:\.self){ src in Button{selectedSource=src;showSourceMenu=false;loadStream()}label:{HStack(spacing:8){Circle().fill(sourceStatus[src]==true ? .green:sourceStatus[src]==false ? .red:.gray).frame(width:6,height:6);Text(src.rawValue).font(.system(size:13)).foregroundColor(.white);Spacer();if selectedSource==src{Image(systemName:"checkmark").font(.system(size:11)).foregroundColor(.white)}}.padding(.horizontal,14).padding(.vertical,10).background(RoundedRectangle(cornerRadius:10).fill(selectedSource==src ? .white.opacity(0.15):.white.opacity(0.05)))} } }.padding(18).background(RoundedRectangle(cornerRadius:16).fill(.ultraThinMaterial.opacity(0.95))).overlay(RoundedRectangle(cornerRadius:16).stroke(.white.opacity(0.2),lineWidth:0.5)).frame(width:240) }
    ...
    var settingsPopup: some View {
        VStack(spacing: 12) {
            Text("Cài đặt").font(.system(size: 14, weight: .bold)).foregroundColor(.white)
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ForEach(availableQualities, id: \.self) { q in
                    Button {
                        ...
                        HStack(spacing: 4) {
                            Circle()
                                .fill(selectedQuality == q ? Color.green : Color.clear)
                        ...
                        .padding(.vertical, 8)
                        .background(RoundedRectangle(cornerRadius: 8).fill(selectedQuality == q ? Color.white.opacity(0.15) : Color.white.opacity(0.05)))
                    }
                }
            }
            ...
            HStack(spacing: 8) {
                ForEach(["0.5x", "1.0x", "1.5x", "2.0x"], id: \.self) { s in
                    Button {
                        ...
                        Text(s).font(.system(size: 12)).foregroundColor(.white).padding(.horizontal, 12).padding(.vertical, 6).background(Capsule().fill(.white.opacity(0.1)))
                    }
                }
            }
        }
    ...
    func lockToLandscape() { if isOrientationLocked { if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene { ws.requestGeometryUpdate(.iOS(interfaceOrientations: .landscapeRight)) } } }
...
struct CastSheetView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Text("Phát đến thiết bị").font(.system(size: 18, weight: .bold)).foregroundColor(.white)
                    Spacer()
                    if isScanning { ProgressView().tint(.white).scaleEffect(0.8) }
                    Button { scanDevices() } label: {
                        Image(systemName: "arrow.clockwise").font(.system(size: 14)).foregroundColor(.white.opacity(0.7)).padding(8).background(Circle().fill(.white.opacity(0.1)))
                    }
                }
                ...
                if selectedDevice != nil {
                    VStack(spacing: 10) {
                        ...
                        Text("Chế độ").font(.system(size: 13, weight: .medium)).foregroundColor(.white.opacity(0.6)).frame(maxWidth: .infinity, alignment: .leading).padding(.horizontal, 20).padding(.top, 8)
                        HStack(spacing: 12) { ForEach(CastMode.allCases, id: \.self) { mode in modeButton(mode) } }.padding(.horizontal, 20)
                        Button { startCasting() } label: {
                            HStack(spacing: 8) {
                                Image(systemName: "antenna.radiowaves.left.and.right").font(.system(size: 14))
                                Text("Bắt đầu phát").font(.system(size: 15, weight: .semibold))
                            }
                            .foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 14)
                            .background(LinearGradient(colors: [Color.blue.opacity(0.8), Color.blue.opacity(0.6)], startPoint: .leading, endPoint: .trailing))
            ...
            .background(RoundedRectangle(cornerRadius: 28).fill(.ultraThinMaterial.opacity(0.98)).overlay(RoundedRectangle(cornerRadius: 28).stroke(.white.opacity(0.12), lineWidth: 0.5)))
    ...
    func deviceCard(_ device: CastDevice) -> some View {
        Button {
            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { selectedDevice = selectedDevice?.id == device.id ? nil : device }
        } label: {
            HStack(spacing: 14) {
                ZStack {
                    Circle().fill(.white.opacity(selectedDevice?.id == device.id ? 0.15 : 0.08)).frame(width: 46, height: 46)
                    Image(systemName: device.icon).font(.system(size: 18)).foregroundColor(selectedDevice?.id == device.id ? .white : .white.opacity(0.7))
                }
                .overlay(Circle().stroke(selectedDevice?.id == device.id ? Color.blue.opacity(0.6) : .white.opacity(0.08), lineWidth: selectedDevice?.id == device.id ? 2 : 1))
                VStack(alignment: .leading, spacing: 2) {
                    Text(device.name).font(.system(size: 14, weight: .medium)).foregroundColor(.white)
                    HStack(spacing: 4) {
                        Text(device.type.rawValue).font(.system(size: 11)).foregroundColor(.white.opacity(0.5))
                        HStack(spacing: 2) { ForEach(0..<4, id: \.self) { i in Circle().fill(i < device.signalStrength ? Color.green.opacity(0.7) : .white.opacity(0.15)).frame(width: 3, height: 3) } }
                    }
                }
                Spacer()
                if selectedDevice?.id == device.id { Image(systemName: "checkmark.circle.fill").font(.system(size: 22)).foregroundColor(.blue).transition(.scale.combined(with: .opacity)) }
            }
            .padding(12)
            .background(RoundedRectangle(cornerRadius: 14).fill(selectedDevice?.id == device.id ? .white.opacity(0.08) : .white.opacity(0.03)).overlay(RoundedRectangle(cornerRadius: 14).stroke(selectedDevice?.id == device.id ? Color.blue.opacity(0.3) : .white.opacity(0.05), lineWidth: 0.5)))
        }
    }
    ...
    func modeButton(_ mode: CastMode) -> some View {
        Button {
            ...
            VStack(spacing: 6) {
                Image(systemName: mode == .remote ? "iphone.gen1" : "rectangle.split.2x1").font(.system(size: 20))
                Text(mode.rawValue).font(.system(size: 11, weight: .medium))
            }
            .foregroundColor(selectedMode == mode ? .white : .white.opacity(0.5)).frame(maxWidth: .infinity).padding(.vertical, 12)
            .background(RoundedRectangle(cornerRadius: 12).fill(selectedMode == mode ? Color.blue.opacity(0.3) : .white.opacity(0.05)).overlay(RoundedRectangle(cornerRadius: 12).stroke(selectedMode == mode ? Color.blue.opacity(0.4) : .white.opacity(0.08), lineWidth: 0.5)))
        }
    }
    
    func scanDevices() {
        isScanning = true; devices.removeAll()
        let routeDetector = AVRouteDetector(); routeDetector.isRouteDetectionEnabled = true
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            var foundDevices: [CastDevice] = []
            if routeDetector.multipleRoutesDetected { foundDevices.append(CastDevice(name: "Apple TV / Smart TV", icon: "appletv.fill", type: .airplay, signalStrength: 4)) }
            ...
            self.devices = foundDevices; self.isScanning = false
        }
    }
    
    func startCasting() {
        guard let device = selectedDevice else { return }
        castDeviceName = device.name; isCasting = true
        player.allowsExternalPlayback = true; player.usesExternalPlaybackWhileExternalScreenIsActive = true
        EmmewCastManager.shared.startCasting(with: player, deviceName: device.name)
        dismiss()
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { showRemote = true }
    }
}
...
struct CastRemoteView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(colors: [Color(white: 0.12), Color(white: 0.04), .black], startPoint: .top, endPoint: .bottom).ignoresSafeArea().overlay(.ultraThinMaterial.opacity(0.05))
            VStack(spacing: 0) {
                HStack {
                    HStack(spacing: 6) {
                        Circle().fill(Color.green).frame(width: 6, height: 6).overlay(Circle().fill(Color.green.opacity(0.4)).frame(width: 12, height: 12).scaleEffect(isPlaying ? 1.5 : 1).opacity(isPlaying ? 0.6 : 0).animation(.easeInOut(duration: 1.5).repeatForever(autoreverses: true), value: isPlaying))
                        Text("Đang phát trên \(castDeviceName)").font(.system(size: 11)).foregroundColor(.white.opacity(0.6))
                    }
                    Spacer()
                    Button { stopCasting() } label: { Text("Ngắt kết nối").font(.system(size: 11, weight: .medium)).foregroundColor(.red.opacity(0.8)).padding(.horizontal, 12).padding(.vertical, 5).background(Capsule().fill(.white.opacity(0.1))) }
                }
                .padding(.horizontal, 20).padding(.top, 50)
                Spacer()
                VStack(spacing: 16) {
                    if let url = posterURL { CachedAsyncImage(url: url).aspectRatio(2/3, contentMode: .fit).frame(height: 220).clipShape(RoundedRectangle(cornerRadius: 20)).shadow(color: .white.opacity(0.15), radius: 20, y: -5).overlay(RoundedRectangle(cornerRadius: 20).stroke(.white.opacity(0.15), lineWidth: 1)) }
                    VStack(spacing: 4) { Text(movieTitle).font(.system(size: 20, weight: .bold, design: .serif)).foregroundColor(.white).multilineTextAlignment(.center); if !episodeInfo.isEmpty { Text(episodeInfo).font(.system(size: 12)).foregroundColor(.white.opacity(0.5)) } }.padding(.horizontal, 40)
                }
                ...
                Spacer()
                VStack(spacing: 6) {
                    Slider(value: $currentTime, in: 0...max(duration, 1)) { editing in if !editing { player.seek(to: CMTime(seconds: currentTime, preferredTimescale: 600)) } }.accentColor(.white).padding(.horizontal, 30)
                    HStack { Text(formatTime(currentTime)).font(.system(size: 11, design: .monospaced)).foregroundColor(.white.opacity(0.5)); Spacer(); Text("-" + formatTime(max(duration - currentTime, 0))).font(.system(size: 11, design: .monospaced)).foregroundColor(.white.opacity(0.5)) }.padding(.horizontal, 34)
                }
                HStack(spacing: 50) {
                    Button { showAudioMenu.toggle() } label: { VStack(spacing: 4) { Image(systemName: "waveform").font(.system(size: 22)); Text(selectedAudio).font(.system(size: 10)) }.foregroundColor(.white.opacity(0.8)) }
                    Button { let newTime = max(currentTime - 10, 0); player.seek(to: CMTime(seconds: newTime, preferredTimescale: 600)); currentTime = newTime } label: { Image(systemName: "gobackward.10").font(.system(size: 28)).foregroundColor(.white.opacity(0.9)) }
                    Button { if player.rate == 0 { player.play(); isPlaying = true } else { player.pause(); isPlaying = false } } label: { Image(systemName: isPlaying ? "pause.fill" : "play.fill").font(.system(size: 40)).foregroundColor(.white).padding(20).background(Circle().fill(.ultraThinMaterial.opacity(0.3))).overlay(Circle().stroke(.white.opacity(0.15), lineWidth: 0.5)) }
                    Button { let newTime = min(currentTime + 10, duration); player.seek(to: CMTime(seconds: newTime, preferredTimescale: 600)); currentTime = newTime } label: { Image(systemName: "goforward.10").font(.system(size: 28)).foregroundColor(.white.opacity(0.9)) }
                    Button { withAnimation(.easeInOut(duration: 0.3)) { showInfo.toggle() } } label: { VStack(spacing: 4) { Image(systemName: "info.circle").font(.system(size: 22)); Text("Info").font(.system(size: 10)) }.foregroundColor(.white.opacity(0.8)) }
                }
                .padding(.top, 10)
                if showAudioMenu { VStack(spacing: 8) { ForEach(["Vietsub", "Thuyết minh", "Lồng tiếng", "Original"], id: \.self) { audio in Button { selectedAudio = audio; showAudioMenu = false } label: { HStack { Text(audio).font(.system(size: 14)).foregroundColor(.white); Spacer(); if selectedAudio == audio { Image(systemName: "checkmark").font(.system(size: 12)).foregroundColor(.white) } }.padding(.horizontal, 16).padding(.vertical, 10).background(RoundedRectangle(cornerRadius: 10).fill(selectedAudio == audio ? .white.opacity(0.15) : .white.opacity(0.05))) } } }.padding(14).background(RoundedRectangle(cornerRadius: 16).fill(.ultraThinMaterial.opacity(0.95))).overlay(RoundedRectangle(cornerRadius: 16).stroke(.white.opacity(0.15), lineWidth: 0.5)).padding(.horizontal, 40) }

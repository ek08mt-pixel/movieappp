...
case .addon:
    // Debug: in ra addon đang dùng
    let enabledAddons = AddonManager.shared.addons.filter { $0.enabled }
    print("🔍 [Addon] Số addon đã bật: \(enabledAddons.count)")
    for addon in enabledAddons {
        print("🔍 [Addon] Đang thử: \(addon.manifest.name) - \(addon.url)")
    }
    
    let streamURL = try await AddonManager.shared.fetchBestStream(movieTitle: movieTitle, mediaType: mediaType)
    guard let url = streamURL else { throw StreamError.noStreamAvailable }
    print("✅ [Addon] Tìm thấy stream: \(url)")
    await MainActor.run {

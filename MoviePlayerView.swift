...
class NguonCService {
    ...
    func fetchEmbed(title: String, episode: Int, movieId: Int, mediaType: String?) async throws -> (URL, String) {
        ...
        for item in finalSlugs {
            ...
            do {
                ...
                struct R: Codable { let movie: M? }
                struct M: Codable { let name: String?; let episodes: [S]? }
                struct S: Codable { let server_name: String?; let items: [I]? }
                struct I: Codable { let name: String?; let embed: String? }
                if let response = try? JSONDecoder().decode(R.self, from: dd), let servers = response.movie?.episodes {
                    for server in servers {
    ...
    private func findAllSlugs(title: String) async throws -> [(slug: String, name: String, originalName: String)] {
        ...
        struct R: Codable { let items: [I]? }
        struct I: Codable { let slug: String?; let name: String?; let original_name: String? }
        if let response = try? JSONDecoder().decode(R.self, from: data), let items = response.items, !items.isEmpty {
            return items.compactMap { i in
                guard let slug = i.slug else { return nil }
                return (slug, i.name ?? "", i.original_name ?? "")
            }
        }
        return []
    }

...
struct MoviePlayerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            else if let err = errorMessage { ErrorView2(err: err, status: $sourceStatus, sel: $selectedSource, url: streamURL, src: sources, retry: { loadSource() }, pick: { i in selectedSource = i; loadSource() }) }
        ...
        .task { loadSource() }
        .sheet(isPresented: $showSourceMenu) { SourceMenu2(src: sources, sel: $selectedSource, status: $sourceStatus, pick: { loadSource() }) }
    }
    ...
    func loadDirect() { Task {
        do { let id = try await fetchIMDb(); let url = selectedSource == 0 ? try await fetchNTL(id) : try await MediaFusionManager.shared.getBestURL(imdbId: id)
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                ForEach(0..<src.count, id: \.self) { i in
                    Button { pick(i) } label: {
                        VStack(spacing: 4) {
                            Image(systemName: status[i] == true ? "checkmark.circle.fill" : (status[i] == false ? "xmark.circle.fill" : "circle")).font(.system(size: 18)).foregroundColor(status[i] == true ? .green : (status[i] == false ? .red : .gray))
                            Text(src[i].0).font(.system(size: 9)).foregroundColor(.white).lineLimit(1)
                        }.frame(maxWidth: .infinity).padding(.vertical, 8).background(RoundedRectangle(cornerRadius: 8).fill(sel == i ? .white.opacity(0.15) : .ultraThinMaterial))
                    }
                }
            }.padding(.horizontal)
            HStack {
...
struct PlayerView2: View {
    ...
    var body: some View {
        FullScreenPlayer(player: player).ignoresSafeArea().overlay(alignment: .topTrailing) {
            Button(action: tap) { ZStack { Circle().fill(.ultraThinMaterial).frame(width: 36, height: 36); Text(name).font(.system(size: 8)).foregroundColor(.white) } }.padding(.top, 50).padding(.trailing, 16)
        }
    }
}
...
struct WebView2: View {
    ...
    var body: some View {
        FullScreenWebView(urlString: url).ignoresSafeArea().overlay(alignment: .topTrailing) {
            Button(action: tap) { ZStack { Circle().fill(.ultraThinMaterial).frame(width: 36, height: 36); Text(name).font(.system(size: 8)).foregroundColor(.white) } }.padding(.top, 50).padding(.trailing, 16)
        }
    }
}

struct SourceMenu2: View {
    ...
    var body: some View {
        NavigationStack { ZStack { Color.black.opacity(0.95).ignoresSafeArea()
            ScrollView { LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                ForEach(0..<src.count, id: \.self) { i in Button { sel = i; pick(); d() } label: {
                    VStack(spacing: 6) {
                        Image(systemName: status[i] == true ? "checkmark.circle.fill" : (status[i] == false ? "xmark.circle.fill" : "play.circle.fill")).font(.system(size: 22)).foregroundColor(status[i] == true ? .green : (status[i] == false ? .red : .white.opacity(0.6)))
                        Text(src[i].0).font(.system(size: 9)).foregroundColor(.white).lineLimit(2)
                        if status[i] == true { Text("OK").font(.system(size: 8)).foregroundColor(.green) } else if status[i] == false { Text("Lỗi").font(.system(size: 8)).foregroundColor(.red) }
                    }.frame(maxWidth: .infinity).padding(.vertical, 12).background(RoundedRectangle(cornerRadius: 12).fill(sel == i ? .white.opacity(0.1) : .ultraThinMaterial))
                } }
            }.padding() } }.navigationTitle("Chọn nguồn").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .navigationBarTrailing) { Button("Xong") { d() }.foregroundColor(.white) } }
        }
    }
}
...
struct FullScreenWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        let wv = WKWebView(frame: .zero, configuration: config); wv.backgroundColor = .black; wv.isOpaque = false; wv.scrollView.contentInsetAdjustmentBehavior = .never
        wv.customUserAgent = "Mozilla/5.0"

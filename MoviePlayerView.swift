...
struct MoviePlayerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if showControls && errorMessage==nil && !isLoading && !showOverlay && !showSourceMenu && !showSettings && !showAudioPopup {
                ...
                VStack{Spacer();
                    VStack(spacing:0){
                        VStack(spacing:6){
                            // Thanh tiến trình thủy tinh - bỏ nút tròn
        ...
        .statusBarHidden()
        .gesture(DragGesture(minimumDistance:20).onChanged{v in let h=UIScreen.main.bounds.height;let t=max(h*0.6,h-250);if !showOverlay&&v.translation.height < -40&&v.startLocation.y>t{showOverlay=true;overlayOffset=300};if showOverlay&&v.translation.height>40{overlayOffset=max(0,v.translation.height)}}.onEnded{v in if showOverlay&&v.translation.height>100{closeOverlay()}else if showOverlay{withAnimation(.spring(response:0.3,dampingFraction:0.8)){overlayOffset=0}}})
        .task{loadStream()}

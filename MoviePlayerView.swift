...
enum MovieSource: String, CaseIterable { case phimapi="Emew 1", sofaflix="Emew 2", nguonc="Emew 3", vsmov="Emew 4", stravo="Emew 5" }
...
struct MoviePlayerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            CustomPlayerVC(player: player, pipController: $pipController).ignoresSafeArea()
                ...
                .onDisappear { saveProgress(); player.pause(); player.replaceCurrentItem(with: nil); controlsTimer?.invalidate(); unlockOrientation(); if watchService.isInRoom { watchService.leaveRoom() } }
                .onTapGesture { if showOverlay { closeOverlay() } else { toggleControls() } }
                .onChange(of: player.rate) { newRate in if watchService.isInRoom && watchService.isHost { watchService.sendPlaybackState(action: newRate > 0 ? "play" : "pause", time: currentTime) } }
                .onChange(of: watchService.remoteState?.timestamp) { _ in
                    ...
                    let targetTime = CMTime(seconds: state.time, preferredTimescale: 600)
                    if state.action == "play" { player.seek(to: targetTime); player.play() }
                    else if state.action == "pause" { player.seek(to: targetTime); player.pause() }
                    else if state.action == "seek" { player.seek(to: targetTime) }
                }
            ...
            if showControls && errorMessage == nil && !isLoading && !showOverlay && !showSourceMenu && !showSettings && !showSubtitlePopup && !showAudioPopup && !showWatchTogether {
                ...
                VStack{Spacer(); VStack(spacing:8){Slider(value:$currentTime,in:0...max(duration,1)){e in isSeeking=e; if !e{player.seek(to:CMTime(seconds:currentTime,preferredTimescale:600)); if watchService.isInRoom && watchService.isHost { watchService.sendPlaybackState(action: "seek", time: currentTime) }}}.accentColor(.white).padding(.horizontal,30); HStack{Text(formatTime(currentTime)).font(.caption2).foregroundColor(.white.opacity(0.7));Spacer();Text(formatTime(duration)).font(.caption2).foregroundColor(.white.opacity(0.7))}.padding(.horizontal,30)
                    HStack(spacing:0){ Spacer()
                        HStack(spacing:40){ Button{prevEpisode()}label:{Image(systemName:"backward.end.fill").font(.system(size:26)).foregroundColor(.white.opacity(0.9))}; Button{toggleOrientationLock()}label:{Image(systemName:isOrientationLocked ? "lock.rotation":"lock.open.rotation").font(.system(size:26)).foregroundColor(.white.opacity(0.9))}; Button{showSubtitlePopup=true}label:{Image(systemName:"captions.bubble").font(.system(size:26)).foregroundColor(.white.opacity(0.9))}; Button{showAudioPopup=true}label:{Image(systemName:"waveform").font(.system(size:26)).foregroundColor(.white.opacity(0.9))}; Button{nextEpisode()}label:{Image(systemName:"forward.end.fill").font(.system(size:26)).foregroundColor(.white.opacity(0.9))} }
                        Spacer()
            ...
            if showSourceMenu || showSettings || showSubtitlePopup || showAudioPopup {
                ...
                if showSourceMenu { sourcePopup }; if showSettings { settingsPopup }; if showSubtitlePopup { subtitlePopup }; if showAudioPopup { audioPopup }
            }
        }
        .statusBarHidden()
    ...
    // MARK: - Watch Together
    var watchTogetherPanel: some View {
        VStack(spacing: 0) {
            Spacer()
            VStack(spacing: 12) {
                watchHeader
                if watchService.isInRoom { watchRoomView } else { watchJoinView }
            }
            .padding(20)
            .background(RoundedRectangle(cornerRadius: 24).fill(.ultraThinMaterial.opacity(0.95)))
            .overlay(RoundedRectangle(cornerRadius: 24).stroke(.white.opacity(0.2), lineWidth: 1))
            .frame(height: UIScreen.main.bounds.height * 0.55)
        }
    }
    
    var watchHeader: some View {
        HStack {
            Text("Xem chung").font(.headline).foregroundColor(.white)
            if watchService.isInRoom { Text("#\(watchService.currentRoomCode)").font(.caption).foregroundColor(.gray) }
            Spacer()
            Button { showWatchTogether = false } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 20)).foregroundColor(.gray) }
        }
    }
    
    var watchRoomView: some View {
        VStack(spacing: 12) {
            ScrollView(.horizontal) {
                HStack(spacing: 12) {
                    ForEach(watchService.participants, id: \.userId) { p in
                        VStack(spacing: 4) {
                            Circle().fill(.ultraThinMaterial).frame(width: 40, height: 40).overlay(Text(String(p.userName.prefix(1))).foregroundColor(.white).font(.caption))
            ...
            ScrollView {
                VStack(spacing: 8) { ForEach(watchService.messages) { msg in watchChatBubble(msg) } }
            }.frame(height: 150)
            
            HStack(spacing: 8) {
                TextField("Nhắn tin...", text: $watchMessage).textFieldStyle(.plain).foregroundColor(.white).padding(10).background(RoundedRectangle(cornerRadius: 20).fill(.ultraThinMaterial))
                Button { if !watchMessage.isEmpty { watchService.sendMessage(text: watchMessage); watchMessage = "" } } label: { Image(systemName: "paperplane.fill").foregroundColor(.white).padding(8).background(Circle().fill(.blue.opacity(0.5))) }
            }
            Button { watchService.leaveRoom(); showWatchTogether = false } label: { Text("Rời phòng").font(.caption).foregroundColor(.red) }
        }
    }
    
    var watchJoinView: some View {
        VStack(spacing: 12) {
            TextField("Tên của bạn", text: $watchUserName).textFieldStyle(.plain).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(.ultraThinMaterial)).padding(.horizontal, 20)
            Button { if !watchUserName.isEmpty { watchService.createRoom(userName: watchUserName) { _ in } } } label: { Text("Tạo phòng mới").font(.headline).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Capsule().fill(.ultraThinMaterial)).overlay(Capsule().stroke(Color.white.opacity(0.2), lineWidth: 0.5)) }.padding(.horizontal, 20)
            TextField("Mã phòng", text: $joinCode).textFieldStyle(.plain).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(.ultraThinMaterial)).padding(.horizontal, 20).keyboardType(.numberPad)
            if !joinError.isEmpty { Text(joinError).font(.caption).foregroundColor(.red) }
            Button { if !watchUserName.isEmpty && !joinCode.isEmpty { watchService.joinRoom(code: joinCode, userName: watchUserName) { success in if !success { joinError = "Mã phòng không đúng" } else { joinError = "" } } } } label: { Text("Vào phòng").font(.headline).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Capsule().fill(.ultraThinMaterial)).overlay(Capsule().stroke(Color.white.opacity(0.2), lineWidth: 0.5)) }.padding(.horizontal, 20)
        }
    }
    
    func watchChatBubble(_ msg: WatchTogetherService.ChatMessage) -> some View {
        HStack {
            ...
            VStack(alignment: msg.userId == watchService.userId ? .trailing : .leading, spacing: 2) {
                ...
                Text(msg.text).font(.system(size: 12)).foregroundColor(.white).padding(.horizontal, 10).padding(.vertical, 6).background(RoundedRectangle(cornerRadius: 12).fill(msg.userId == watchService.userId ? .blue.opacity(0.4) : .ultraThinMaterial.opacity(0.5)))
            }
            if msg.userId == watchService.userId { Spacer().frame(width: 40) }
        }
    }
    
    // MARK: - Rest of views (giữ nguyên)
    @ViewBuilder
    ...
    func loadStream(season: Int? = nil, episode: Int? = nil) { let ep = episode ?? episodeNumber ?? 1; let s = season ?? seasonNumber; seasonNumber = s; episodeNumber = ep; if mediaType == "tv" || s != nil { selectedSeasonNumber = s; Task { selectedSeasonDetail = try? await APIService.shared.fetchSeasonDetail(tvId: movieId, seasonNumber: s ?? 1) } }; isLoading = true; errorMessage = nil; sourceStatus[selectedSource] = nil; Task { do { let imdbID = try await fetchIMDB(); switch selectedSource { case .phimapi: let url = try await withCheckedThrowingContinuation { c in PhimAPIService.shared.fetchStream(imdbID: imdbID, tmdbID: movieId, title: movieTitle, mediaType: mediaType, season: s, episode: ep) { c.resume(with: $0) } }; await MainActor.run { player.replaceCurrentItem(with: AVPlayerItem(url: url)); player.play(); hasStartedPlaying = true; sourceStatus[.phimapi] = true; isLoading = false; tryResume() }; saveHistory() case .sofaflix: let url = try await withCheckedThrowingContinuation { c in SofaflixService.shared.fetchStream(imdbID: imdbID, tmdbID: movieId, title: movieTitle, mediaType: mediaType, season: s, episode: ep) { c.resume(with: $0) } }; await MainActor.run { player.replaceCurrentItem(with: AVPlayerItem(url: url)); player.play(); hasStartedPlaying = true; sourceStatus[.sofaflix] = true; isLoading = false; tryResume() }; saveHistory() case .nguonc: let url = try await withCheckedThrowingContinuation { c in NguonCService.shared.fetchStream(imdbID: imdbID, title: movieTitle, season: s, episode: ep) { c.resume(with: $0) } }; await MainActor.run { nguonCEmbedURL = url; nguonCEpisodeName = "\(movieTitle) - Tập \(ep)"; isLoading = false; sourceStatus[.nguonc] = true; showNguonCWebView = true } case .vsmov: let url = try await withCheckedThrowingContinuation { c in VSMOVService.shared.fetchStream(imdbID: imdbID, title: movieTitle, season: s, episode: ep) { c.resume(with: $0) } }; await MainActor.run { player.replaceCurrentItem(with: AVPlayerItem(url: url)); player.play(); hasStartedPlaying = true; sourceStatus[.vsmov] = true; isLoading = false; tryResume() }; saveHistory() case .stravo: let url = try await withCheckedThrowingContinuation { c in StravoService.shared.fetchStream(imdbID: imdbID, season: s, episode: ep) { c.resume(with: $0) } }; let asset = AVURLAsset(url: url, options: ["AVURLAssetHTTPHeaderFieldsKey": ["Referer": "https://lok-lok.cc/", "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"]]); await MainActor.run { player.replaceCurrentItem(with: AVPlayerItem(asset: asset)); player.play(); hasStartedPlaying = true; sourceStatus[.stravo] = true; isLoading = false; tryResume() }; saveHistory() } } catch { await MainActor.run { sourceStatus[selectedSource] = false; errorMessage = error.localizedDescription; isLoading = false } } } }

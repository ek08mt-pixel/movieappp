...
struct MoviePlayerView: View {
        ...
        ZStack {
            ...
            CustomPlayerVC(player: player, pipController: $pipController, gravity: selectedVideoGravity).ignoresSafeArea()
                .onAppear {
                    ...
                    lockToLandscape()
                    if let savedIndex = UserDefaults.standard.value(forKey: "lastAudioIndex_\(movieId)") as? Int {
                        selectedServerIndex = savedIndex
                    }
                    if let savedLabel = UserDefaults.standard.string(forKey: "lastAudioLabel_\(movieId)") {
    ...
    DispatchQueue.main.asyncAfter(deadline:.now()+0.3){dismiss()}
                ...
                if isCasting {
                    VStack { Spacer(); HStack { Spacer()
                        HStack(spacing: 6) {
                            ...
                            Text("Đang phát trên \(castDeviceName)").font(.system(size: 10)).foregroundColor(.white.opacity(0.7))
                        }
                        .padding(.horizontal, 12).padding(.vertical, 6).background(Capsule().fill(.ultraThinMaterial.opacity(0.5)))
                        .padding(.trailing, 20).padding(.bottom, 100)
                    }}
                }
            }
            if showNextEpisodePopup { /* giữ nguyên */ }
            if showOverlay { youtubeOverlay }
            if showSourceMenu || showSettings || showAudioPopup {
                Color.black.opacity(0.3).ignoresSafeArea().onTapGesture { showSourceMenu = false; showSettings = false; showAudioPopup = false }
        ...
        .fullScreenCover(isPresented: $showRemoteControl) {
            CastRemoteView(movieTitle: movieTitle, episodeInfo: episodeInfo, posterURL: posterURL, castDeviceName: castDeviceName, player: player, currentTime: $currentTime, duration: $duration, isCasting: $isCasting)
        }
    ...
    func qualityURL(from originalURL: URL, quality: String) -> URL {
        ...
        let qualityMap: [String: String] = ["4K": "2160", "2880p": "2880", "2160p": "2160", "1440p": "1440", "1080p": "1080", "720p": "720", "480p": "480"]
        guard let targetRes = qualityMap[quality] else { return originalURL }
        for q in ["2160", "2880", "1440", "1080", "720", "480", "4k"] {
            if urlStr.contains(q) { return URL(string: urlStr.replacingOccurrences(of: q, with: targetRes, options: .caseInsensitive)) ?? originalURL }
        }
    ...
    func applyQuality(_ quality: String) {
        ...
        player.seek(to: CMTime(seconds: savedTime, preferredTimescale: 600)) { _ in player.play() }
    }
    ...
    Picker("Server", selection: $selectedServerIndex) { ForEach(0..<phimapiServers.count, id: \.self) { i in Text(phimapiServers[i]).tag(i) } }
    .pickerStyle(.segmented).onChange(of: selectedServerIndex) { _ in let savedTime = currentTime; loadStream(season: seasonNumber, episode: episodeNumber, resumeAt: savedTime) }.padding(.bottom, 8)
    }
    ...
    var settingsPopup: some View { VStack(spacing: 12) { Text("Cài đặt").font(.system(size: 14, weight: .bold)).foregroundColor(.white); Text("Chất lượng").font(.system(size: 11)).foregroundColor(.white.opacity(0.6)); LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) { ForEach(availableQualities, id: \.self) { q in Button { applyQuality(q); showSettings = false } label: { HStack(spacing: 4) { Circle().fill(selectedQuality == q ? Color.green : Color.clear).frame(width: 6, height: 6); Text(q).font(.system(size: 12)).foregroundColor(selectedQuality == q ? .white : .white.opacity(0.6)) }.frame(maxWidth: .infinity).padding(.vertical, 8).background(RoundedRectangle(cornerRadius: 8).fill(selectedQuality == q ? Color.white.opacity(0.15) : Color.white.opacity(0.05))) } } }; Divider().background(Color.white.opacity(0.1)); Text("Tốc độ").font(.system(size: 11)).foregroundColor(.white.opacity(0.6)); HStack(spacing: 8) { ForEach(["0.5x", "1.0x", "1.5x", "2.0x"], id: \.self) { s in Button { player.rate = Float(s.replacingOccurrences(of: "x", with: "")) ?? 1.0; showSettings = false } label: { Text(s).font(.system(size: 12)).foregroundColor(.white).padding(.horizontal, 12).padding(.vertical, 6).background(Capsule().fill(.white.opacity(0.1))) } } } }.padding(18).background(RoundedRectangle(cornerRadius: 16).fill(.ultraThinMaterial.opacity(0.95))).overlay(RoundedRectangle(cornerRadius: 16).stroke(.white.opacity(0.2), lineWidth: 0.5)).frame(width: 260) }
    ...
    func loadStream(season: Int? = nil, episode: Int? = nil, resumeAt: Double? = nil) {
        if let s = season { seasonNumber = s }; if let e = episode { episodeNumber = e }
        ...
        Task { do { let imdbID = try await fetchIMDB()
                ...
                case .phimapi: 
    let result = try await withCheckedThrowingContinuation { c in PhimAPIService.shared.fetchStream(imdbID: imdbID, tmdbID: movieId, title: movieTitle, mediaType: mediaType, season: s, episode: ep, serverIndex: selectedServerIndex) { c.resume(with: $0) } }
    await MainActor.run { phimapiServers = result.1; currentStreamURL = result.0; selectedQuality = detectQuality(from: result.0)
        let item = AVPlayerItem(url: result.0); player.replaceCurrentItem(with: item)
        if let resumeTime = resumeAt { player.seek(to: CMTime(seconds: resumeTime, preferredTimescale: 600)) { _ in player.play() } } else { player.play() }
        hasStartedPlaying = true; sourceStatus[.phimapi] = true; isLoading = false; tryResume()
    }; saveHistory()
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Text("Phát đến thiết bị").font(.system(size: 18, weight: .bold)).foregroundColor(.white); Spacer()
                    if isScanning { ProgressView().tint(.white).scaleEffect(0.8) }
                    Button { scanDevices() } label: { Image(systemName: "arrow.clockwise").font(.system(size: 14)).foregroundColor(.white.opacity(0.7)).padding(8).background(Circle().fill(.white.opacity(0.1))) }
                }.padding(.horizontal, 20).padding(.top, 12)
                ScrollView { VStack(spacing: 10) { ForEach(devices) { device in deviceCard(device) } }.padding(.horizontal, 20).padding(.top, 12) }.frame(maxHeight: 280)
                if selectedDevice != nil { VStack(spacing: 10) { Divider().background(Color.white.opacity(0.15)).padding(.horizontal, 20); Text("Chế độ").font(.system(size: 13, weight: .medium)).foregroundColor(.white.opacity(0.6)).frame(maxWidth: .infinity, alignment: .leading).padding(.horizontal, 20).padding(.top, 8); HStack(spacing: 12) { ForEach(CastMode.allCases, id: \.self) { mode in modeButton(mode) } }.padding(.horizontal, 20); Button { startCasting() } label: { HStack(spacing: 8) { Image(systemName: "antenna.radiowaves.left.and.right").font(.system(size: 14)); Text("Bắt đầu phát").font(.system(size: 15, weight: .semibold)) }.foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 14).background(LinearGradient(colors: [Color.blue.opacity(0.8), Color.blue.opacity(0.6)], startPoint: .leading, endPoint: .trailing)).clipShape(RoundedRectangle(cornerRadius: 14)) }.padding(.horizontal, 20).padding(.bottom, 30) } }
                Spacer().frame(height: 20)
            }.background(RoundedRectangle(cornerRadius: 28).fill(.ultraThinMaterial.opacity(0.98)).overlay(RoundedRectangle(cornerRadius: 28).stroke(.white.opacity(0.12), lineWidth: 0.5))).shadow(color: .black.opacity(0.5), radius: 30, y: -10)
...
extension VideoGravityMode {
    var avGravity: AVLayerVideoGravity { switch self { case .fit: return .resizeAspect; case .fill: return .resizeAspectFill; case .stretch: return .resize } }
}

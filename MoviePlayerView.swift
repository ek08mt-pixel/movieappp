...
func audioOptions() -> [String] { 
    ...
    if selectedSource == .phimapi { 
        if let idx = phimapiServers.firstIndex(of: label) {
            ...
            if let idx = phimapiServers.firstIndex(where: { $0.localizedCaseInsensitiveContains(label) }) {
                selectedServerIndex = idx
                UserDefaults.standard.set(idx, forKey: "lastAudioIndex_\(movieId)")
                let st = currentTime
                loadStream(season: seasonNumber, episode: episodeNumber, resumeAt: st)
            } else {
                // Không tìm thấy, dùng index 1
                selectedServerIndex = 1
                UserDefaults.standard.set(1, forKey: "lastAudioIndex_\(movieId)")
                let st = currentTime
                loadStream(season: seasonNumber, episode: episodeNumber, resumeAt: st)
            }
        }

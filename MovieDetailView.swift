                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        HStack(alignment: .top, spacing: 14) {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                Button { showFullOverview.toggle() } label: {
                        ...
                        HStack(spacing: 10) {
                            ...
                            NavigationLink(destination: AskAIView(movie: movie).toolbar(.hidden, for: .tabBar)) {
                                Label("Hỏi AI", systemImage: "brain")
                                    .frame(maxWidth: .infinity).padding(.vertical, 10)
                                    .background(.ultraThinMaterial)
                                    .overlay(RoundedRectangle(cornerRadius: 20).stroke(Color.white.opacity(0.15), lineWidth: 0.5))
                                    .clipShape(Capsule()).foregroundColor(.white)
                                    ...
                                    HStack(spacing: 10) {
                                        ForEach(vm.seasons) { s in
                                            Button { selectedSeason = s } label: {
                                                VStack(spacing: 6) {
                                                    Text(s.name).font(.system(size: 13, weight: selectedSeason?.id == s.id ? .bold : .medium))
                                                        .foregroundColor(selectedSeason?.id == s.id ? .white : .gray)
                                                    Text("\(s.episodeCount) tập").font(.system(size: 10)).foregroundColor(.gray)
                                                }
                                                .padding(.vertical, 10).padding(.horizontal, 16)
                                                .background(RoundedRectangle(cornerRadius: 14).fill(selectedSeason?.id == s.id ? AnyShapeStyle(.ultraThinMaterial) : AnyShapeStyle(.white.opacity(0.04))))
                                            }
                                        }
                                    }
                                }
                        ...
                        if !isTVShow, let r = vm.detail?.runtime, r > 0 {
                            HStack(spacing: 12) {
                                Label("\(r) phút", systemImage: "clock.fill").font(.system(size: 11)).foregroundColor(.gray)
                                if let g = vm.detail?.genres, !g.isEmpty {
                                    Text(g.prefix(3).map{$0.name}.joined(separator: " • ")).font(.system(size: 11)).foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        if !vm.images.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                ScrollView(.horizontal) {
                                    HStack(spacing: 8) {
                                        ForEach(vm.images.prefix(8), id: \.self) { u in
                                            CachedAsyncImage(url: u).aspectRatio(16/9, contentMode: .fill)
                                                .frame(width: 180, height: 100).clipShape(RoundedRectangle(cornerRadius: 10))
                                        }
                                    }
                                }
                            }
                                ...
                                HStack(spacing: 16) {
                                    ForEach(vm.actors.prefix(15)) { a in
                                        NavigationLink(destination: ActorDetailView(actor: a)) {
                                            VStack(spacing: 6) {
                                                CachedAsyncImage(url: a.profileURL).aspectRatio(contentMode: .fill)
                                                    .frame(width: 60, height: 60).clipShape(Circle())
                                                Text(a.name).font(.system(size: 10)).foregroundColor(.white).lineLimit(1).frame(width: 60)
                                            }
                                        }
                                    }
                                }
                                ...
                                LazyHStack(spacing: 12) {
                                    ForEach(vm.similar.prefix(12)) { m in
                                        NavigationLink(destination: MovieDetailView(movie: m)) {
                                            VStack(spacing: 6) {
                                                CachedAsyncImage(url: m.posterURL).aspectRatio(2/3, contentMode: .fill)
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            HStack(spacing: 8) {
                                TextField("Viết bình luận...", text: $commentText).textFieldStyle(.plain).foregroundColor(.white)
                                    .padding(10).background(RoundedRectangle(cornerRadius: 10).fill(.ultraThinMaterial))
                                Button { if !commentText.isEmpty { comments.append(commentText); commentText = "" } } label: {
                                    Text("Gửi").font(.caption).fontWeight(.bold).foregroundColor(.white.opacity(0.7))
                                }
                            }
            ...
            VStack(spacing: 0) {
                ...
                TabView {
                    ForEach(images, id: \.self) { url in
                        CachedAsyncImage(url: url).aspectRatio(contentMode: .fit).frame(maxWidth: .infinity, maxHeight: .infinity)

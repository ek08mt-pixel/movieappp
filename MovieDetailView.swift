...
.frame(maxWidth: .infinity, alignment: .center)
                        HStack(spacing: 10) {
                            Button { if !vm.seasons.isEmpty { playSeason = 1; playEpisode = 1 } else { playSeason = nil; playEpisode = nil }; presentPlayer() } label: { Label("Xem", systemImage: "play.fill").frame(maxWidth: .infinity).padding(.vertical, 10).background(.ultraThinMaterial).overlay(RoundedRectangle(cornerRadius: 20).stroke(Color.white.opacity(0.15), lineWidth: 0.5)).clipShape(Capsule()).foregroundColor(.white).font(.system(size: 12, weight: .semibold)) }

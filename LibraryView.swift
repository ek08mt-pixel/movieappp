    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    HStack(spacing: 0) {
                        tabButton(.watched); tabButton(.saved); tabButton(.downloads)
                    }
                    .padding(4).background(Capsule().fill(.ultraThinMaterial.opacity(0.2)).overlay(Capsule().stroke(Color.white.opacity(0.08), lineWidth: 0.5)))
                    ...
                    if selectedTab == .downloads {
                        ...
                        VStack(spacing: 12) {
                            Image(systemName: selectedTab == .saved ? "bookmark.slash" : "eye.slash").font(.system(size: 50)).foregroundColor(.gray)
                            Text(selectedTab == .saved ? "Chưa có phim đã lưu" : "Chưa có phim đã xem").foregroundColor(.gray)
                        ...
                        ScrollView {
                            LazyVStack(spacing: 12) { ForEach(currentMovies) { movie in watchHistoryRow(movie) } }
                            .padding(.horizontal, 16).padding(.top, 12).padding(.bottom, 100)
                        }
                    }
                }
            ...
            .fullScreenCover(isPresented: $showOfflinePlayer) {
                if let url = offlineURL { DownloadedPlayerView(url: url, title: offlineTitle) }
            }
        }
    }
    ...
    var downloadsView: some View {
        Group {
            if dm.downloads.isEmpty {
                ...
                ScrollView {
                    LazyVStack(spacing: 12) { ForEach(dm.downloads) { item in downloadRow(item) } }
                    .padding(.horizontal, 16).padding(.top, 12).padding(.bottom, 100)
                }
            }
        }
    ...
    func downloadRow(_ item: DownloadItem) -> some View {
        HStack(spacing: 12) {
            if let path = item.posterPath, let url = URL(string: "https://image.tmdb.org/t/p/w200\(path)") {
                CachedAsyncImage(url: url).aspectRatio(2/3, contentMode: .fill).frame(width: 60, height: 90).clipShape(RoundedRectangle(cornerRadius: 10))
            } else {
                RoundedRectangle(cornerRadius: 10).fill(.ultraThinMaterial.opacity(0.3)).frame(width: 60, height: 90)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(item.movieTitle).font(.system(size: 14, weight: .semibold)).foregroundColor(.white).lineLimit(1)
                if let ep = item.episodeNumber, let s = item.seasonNumber {
                    Text("S\(s):E\(ep)").font(.system(size: 11)).foregroundColor(.gray)
                }
                switch item.status {
                case .downloading:
                    VStack(spacing: 4) {
                        ...
                        Text("\(Int(item.progress * 100))%").font(.system(size: 10)).foregroundColor(.gray)
                    }
                case .paused: Text("Đã tạm dừng").font(.system(size: 11)).foregroundColor(.yellow)
                case .completed: Text("Đã tải xong").font(.system(size: 11)).foregroundColor(.green)
                case .failed: Text("Tải thất bại").font(.system(size: 11)).foregroundColor(.red)
                }
            }
            Spacer()
            VStack(spacing: 10) {
                if item.status == .downloading || item.status == .paused {
                    Button { dm.cancel(item.id) } label: {
                        Image(systemName: "xmark.circle.fill").font(.system(size: 14)).foregroundColor(.red.opacity(0.7)).padding(8).background(Circle().fill(.ultraThinMaterial.opacity(0.4)))
                    }
                    ...
                    Button {
                        ...
                        Image(systemName: "play.fill").font(.system(size: 14)).foregroundColor(.white).padding(8).background(Circle().fill(.ultraThinMaterial.opacity(0.4)))
                    }
                }
                Button { dm.delete(item.id) } label: {
                    Image(systemName: "trash.fill").font(.system(size: 12)).foregroundColor(.red.opacity(0.7)).padding(6).background(Circle().fill(.ultraThinMaterial.opacity(0.3)))
                }
            }
        }

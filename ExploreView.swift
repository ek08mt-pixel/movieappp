...
struct ExploreView: View {
    ...
    let posterMap: [String: String] = [
        "Oscar": "/7RyHsO4yDXtBv1zUU3mTpHeQ0d5.jpg",
        "Cannes": "/TU9NIjwzjoKPwQHoHshkFcQUCG.jpg",
        "IMDb Top": "/zfbjgQE1uSd9wiPTX4VzsLi0rGG.jpg",
        "Netflix": "/rAiYTfKGqDCRIIqo664sY9XZIvQ.jpg",
        "Ghibli": "/edv5CZvWj09upOsy2Y6IwDhK8bt.jpg",
        "Marvel": "/or06FN3Dka5tukK1e9sl16pB3iy.jpg",
        "DC": "/nMKdUUepR0i5zn0y1T4CsSB5ecy.jpg",
        "Pixar": "/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg",
        "Disney": "/qJ2tW6WMUDux911B6EMThhKzGYV.jpg",
        "A24": "/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg"
    ]
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        LazyVGrid(columns: columns, spacing: 10) {
                            ForEach(collections, id: \.0) { title, _, tmdbId, type in
                            ...
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    LazyHStack(spacing: 10) {
                                        ForEach(staffMovies.prefix(20)) { movie in
                        ...
                        if !editorMovies.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    LazyHStack(spacing: 10) {
                                        ForEach(editorMovies.prefix(20)) { movie in
                                            NavigationLink(destination: MovieDetailView(movie: movie)) {
                                                CachedAsyncImage(url: movie.posterURL)
                                                    .aspectRatio(2/3, contentMode: .fill)
                                                    .frame(width: 110, height: 165)
                        ...
                        if !hiddenMovies.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    LazyHStack(spacing: 10) {
                                        ForEach(hiddenMovies.prefix(20)) { movie in
                                            NavigationLink(destination: MovieDetailView(movie: movie)) {
                                                CachedAsyncImage(url: movie.posterURL)
                                                    .aspectRatio(2/3, contentMode: .fill)
                                                    .frame(width: 110, height: 165)
...
struct SectionWithSeeAll: View {
    let title: String; let movies: [Movie]
    var body: some View {
...
struct CategoryFullView: View {
    ...
    private let gridColumns = [GridItem(.flexible(), spacing: 10), GridItem(.flexible(), spacing: 10), GridItem(.flexible(), spacing: 10)]
    ...
    var body: some View {
        ...
        .task {
            do {
                movies = try await APIService.shared.fetchMovies(by: category.tmdbId, type: category.type)
            } catch {
                movies = []
            }
            isLoading = false

    ...
    var body: some View {
        HStack(alignment: .center, spacing: 16) {
            ...
            ZStack {
                ...
                HexagonShape()
                    .fill(LinearGradient(colors: [Color(red: 0.2, green: 0.2, blue: 0.2), Color(red: 0.05, green: 0.05, blue: 0.05)], startPoint: .top, endPoint: .bottom))
                    .frame(width: 88, height: 96)
                
                HexagonShape()
                    .stroke(LinearGradient(colors: [.white.opacity(0.9), .white.opacity(0.1), .white.opacity(0.0), .white.opacity(0.3)], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 1.5)
                    .frame(width: 88, height: 96)
        ...
        // === VIỀN MỎNG (0.8) & ĐỨT ĐOẠN TỰ NHIÊN ===
        .overlay(
            RoundedRectangle(cornerRadius: 24)
                .trim(from: 0.05, to: 0.65) // Cắt bỏ 35% viền để tạo đứt đoạn
                .stroke(
                    LinearGradient(
                        stops: [
        ...
        // Lớp Outer Glow mờ tổng thể (Rất nhẹ)

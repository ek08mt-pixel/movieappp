...
struct TimelineView: View {
        ...
        ZStack(alignment: .topLeading) {
            ...
            VStack(spacing: 0) {
                VStack(spacing: 4) {
                    ...
                    Slider(value: $selectedYear, in: 1900...2026, step: 1).accentColor(.white).padding(.horizontal, 30)
                        .onChange(of: selectedYear) { _ in loadMovies() }
                }
                .padding(.top, 90).padding(.bottom, 16)
    ...
    func loadMovies() {
        isLoading = true
        Task { movies = (try? await APIService.shared.discoverMoviesByYear(Int(selectedYear))) ?? []; isLoading = false }
    }
}
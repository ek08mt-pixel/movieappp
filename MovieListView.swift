...
struct MovieListView: View {
                ...
                LazyVGrid(columns: columns, spacing: 14) {
                    ForEach(allMovies) { movie in
                        NavigationLink(destination: MovieDetailView(movie: movie)) {
                            VStack(spacing: 4) {
                                CachedAsyncImage(url: movie.posterURL).aspectRatio(2/3, contentMode: .fill).frame(height: 170).clipShape(RoundedRectangle(cornerRadius: 10))
    ...
    func loadMore() async {
        ...
        if let new = try? await APIService.shared.search(query: q, page: page), !new.isEmpty { allMovies.append(contentsOf: new.filter { !($0.adult ?? false) }) } else { hasMore = false }
        isLoading = false
    }
}
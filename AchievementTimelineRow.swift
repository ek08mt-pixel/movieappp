...
struct AchievementTimelineRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("HÀNH TRÌNH CỦA BẠN").font(.system(size: 11, weight: .semibold, design: .rounded)).tracking(1.2).foregroundColor(Color(red: 0.45, green: 0.45, blue: 0.45)).padding(.leading, 4)
                ...
                ZStack(alignment: .top) {
                    HStack(spacing: spacing + badgeSize) {
                        ForEach(0..<stages.count - 1, id: \.self) { index in
                            ...
                            Rectangle().fill(Color.white.opacity(nextIsActive ? 0.2 : 0.05)).frame(width: 10, height: 1.5)
                        }
                    }
                    .offset(y: 22)
                    ...
                    HStack(spacing: spacing) {
                        ForEach(Array(stages.enumerated()), id: \.element.id) { index, stage in
                            ...
                            VStack(spacing: 8) {
                                ...
                                VStack(spacing: 2) {
                                    Text(stage.title).font(.system(size: 10, weight: .medium, design: .rounded)).fixedSize(horizontal: true, vertical: false).foregroundColor(isActive ? .white : (isCompleted ? Color(red: 0.6, green: 0.6, blue: 0.6) : Color(red: 0.3, green: 0.3, blue: 0.3)))
                                    Text(stage.level).font(.system(size: 9, weight: .regular, design: .rounded)).foregroundColor(isActive ? Color(red: 0.5, green: 0.5, blue: 0.5) : (isCompleted ? Color(red: 0.4, green: 0.4, blue: 0.4) : Color(red: 0.2, green: 0.2, blue: 0.2)))
                                }
                            }
                        }
                    }

...
struct WatchTogetherRoomView: View {
        ...
        VStack(spacing: 24) {
            HStack {
                Button { showCreateRoom = false } label: {
                    Image(systemName: "chevron.left").font(.system(size: 16, weight: .semibold)).foregroundColor(.white).padding(12).background(Circle().fill(Material.ultraThinMaterial.opacity(0.4)))
                }
                Spacer()
                Text("Tạo phòng").font(.title3.bold()).foregroundColor(.white)
                Spacer()
    ...
    // MARK: - Video Controls (giữ nguyên)

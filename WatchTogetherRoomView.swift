...
struct WatchTogetherRoomView: View {
    ...
    // MARK: - In Room (CHỈNH LẠI LANDSCAPE ĐỂ KHÔNG BAO GIỜ BỊ CROP)
        ...
        GeometryReader { geo in
            ZStack {
                ...
                CustomPlayerVC(player: player, pipController: $pipController)
                    .ignoresSafeArea()
                    // Ép tỷ lệ chuẩn. Fit để không bao giờ cắt hình.
                    .aspectRatio(16/9, contentMode: .fit)
                    // Cho video chiếm hết màn hình, nhưng vẫn bị giới hạn bởi aspectRatio ở trên
                ...
                // Chat Panel: Chỉ hiện khi không phải Landscape
                if !isLandscape {
                    VStack(spacing: 0) {
                        Spacer()
                        imessageChatPanel
    ...
    var imessageChatPanel: some View {
        VStack(spacing: 0) {
            ...
            .padding(.horizontal, 12).padding(.top, 8).padding(.bottom, keyboardHeight > 0 ? keyboardHeight + 10 : 20)

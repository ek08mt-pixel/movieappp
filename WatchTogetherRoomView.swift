...
struct WatchTogetherRoomView: View {
    ...
    var lobbyView: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 14) { ForEach(fakeRooms) { room in fakeRoomCard(room) } }.padding(.horizontal, 16).padding(.bottom, 120)
            }
        }
    }
    ...
    func fakeRoomCard(_ room: FakeRoom) -> some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text(room.movieTitle).font(.system(size: 14, weight: .semibold)).foregroundColor(.white).lineLimit(1)
                Text(room.roomName).font(.system(size: 12)).foregroundColor(.white.opacity(0.6)).lineLimit(1)
                HStack(spacing: 8) {
                    HStack(spacing: -10) {
                    ...
                    Text("\(room.viewerCount) người").font(.system(size: 11)).foregroundColor(.white.opacity(0.5))
                }
            }
            Spacer()
            VStack(alignment: .trailing, spacing: 6) {
                ...
                Text(room.currentTime).font(.system(size: 11, weight: .medium, design: .monospaced)).foregroundColor(.white.opacity(0.7))
                if room.isPrivate {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 22)
                .stroke(LinearGradient(colors: [.white.opacity(0.15), .white.opacity(0.03), .clear], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 0.8)
        )
    ...
    var inRoomView: some View {
        GeometryReader { geo in
            ZStack {
                if isLandscape {
                    HStack(spacing: 0) {
                        ZStack { CustomPlayerVC(player: player, pipController: $pipController); videoControlsOverlay }
    ...
    func handleRemoteState() {
        ...
        else if state.action == "seek" { player.seek(to: target); addEventLog("⏩ Tua tới \(formatTime(state.time))") }
    }
    ...
    var dynamicIslandView: some View {
        Button { showEpisodePanel = true } label: {
            HStack(spacing: 7) {
                Image(systemName: "film").font(.system(size: 10)).foregroundColor(.white.opacity(0.8))
                Text(currentMovieTitle.isEmpty ? "Chọn phim" : currentMovieTitle).font(.system(size: 12, weight: .semibold)).foregroundColor(.white).lineLimit(1)
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Phòng của \(service.currentRoomName)").font(.system(size: 11)).foregroundColor(.white.opacity(0.5))
                }
                Spacer()
                HStack(spacing: 8) {
            ...
            if duration > 0 {
                VStack(spacing: 4) {
                    HStack {
                        Text(formatTime(currentTime)).font(.system(size: 10, design: .monospaced)).foregroundColor(.white.opacity(0.6))
                        Spacer()
                        Text("-\(formatTime(max(duration - currentTime, 0)))").font(.system(size: 10, design: .monospaced)).foregroundColor(.white.opacity(0.4))
                    }
                    GeometryReader { g in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule()
                                ...
                                .frame(width: max(0, g.size.width * CGFloat(currentTime / max(duration, 1))), height: 4)
                            Circle()
                                .fill(.white).frame(width: 14, height: 14)
                                .shadow(color: .white.opacity(0.4), radius: 4)
                                .offset(x: max(0, min(g.size.width - 14, g.size.width * CGFloat(currentTime / max(duration, 1)) - 7)))
                        }
                    }.frame(height: 14)
            ...
            HStack(spacing: 10) {
                ...
                if !watchMessage.isEmpty {
                    Button { sendImessage() } label: {
                        Image(systemName: "arrow.up.circle.fill").font(.system(size: 38)).foregroundColor(.white).shadow(color: .blue.opacity(0.3), radius: 6)
                    }
                }
            }
            .padding(.horizontal, 14).padding(.vertical, 10)
    ...
    func seek(_ s: Double) { let t = max(0, min(currentTime + s, duration)); player.seek(to: CMTime(seconds: t, preferredTimescale: 600)); currentTime = t; if service.isHost { service.sendPlaybackState(action: "seek", time: t) }; addEventLog("⏩ Tua tới \(formatTime(t))") }
            ...
            Task {
                for _ in 0..<3 {
                    if let s = try? await APIService.shared.fetchTVSeasons(tvId: movie.id), !s.isEmpty {
                        await MainActor.run { self.seasons = s }; break
                    }
    ...
    var episodePanel: some View {
        VStack(spacing: 0) {
            Capsule().fill(.gray.opacity(0.5)).frame(width: 36, height: 5).padding(.top, 10)
            Text(currentMovieTitle.isEmpty ? "Chọn tập" : currentMovieTitle).font(.headline).foregroundColor(.white).padding(.vertical, 10)
            if seasons.isEmpty && episodes.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Button { showEpisodePanel = false; showSearchMovie = true } label: { Text("Đổi phim khác").font(.system(size: 13)).foregroundColor(.blue) }
                }.frame(maxHeight: .infinity)
            } else if let selSeason = selectedSeason {
                VStack(alignment: .leading, spacing: 6) {
                    Button {
                        withAnimation { selectedSeason = nil; episodes = [] }
                    } label: {
                        HStack {

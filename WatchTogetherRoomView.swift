...
struct WatchTogetherRoomView: View {
    ...
    var videoControlsOverlay: some View {
        Group {
            if showControls {
                VStack(spacing: 0) {
                    HStack {
                        Button { player.pause(); player.replaceCurrentItem(with: nil); service.leaveRoom() } label: { Image(systemName: "chevron.left").font(.system(size: 14, weight: .semibold)).foregroundColor(.white).padding(6).background(Circle().fill(.ultraThinMaterial.opacity(0.5))) }
                        Spacer()
                        Button { showEpisodePanel = true } label: { Text(displayTitle).font(.system(size: 13, weight: .medium)).foregroundColor(.white).lineLimit(1) }
                        Spacer()
                        Button { toggleOrientation() } label: { Image(systemName: "rotate.right").font(.system(size: 16, weight: .bold)).foregroundColor(.white).padding(6).background(Circle().fill(.ultraThinMaterial.opacity(0.5))) }
                    }.padding(.horizontal, 12).padding(.top, isLandscape ? 4 : 48)
                    Spacer()
                    HStack(spacing: 36) {
    ...
    var imessageChatPanel: some View {
        VStack(spacing: 0) {
            ...
            .padding(.horizontal, 12).padding(.top, 8).padding(.bottom, keyboardHeight > 0 ? keyboardHeight - 20 : 12)

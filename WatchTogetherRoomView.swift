...
struct WatchTogetherRoomView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            showControls = true; resetControlsTimer(); startFakeRoomRefresh()
        ...
        refreshTimer = Timer.scheduledTimer(withTimeInterval: 90, repeats: true) { _ in
            withAnimation(.easeInOut(duration: 0.5)) {
                for _ in 0..<3 {
                    ...
                    fakeRooms[idx].roomName = ["ai đoá ai đóa","gigi ngungục","ziku","Music & Movie","📺 Series Addict","bò cinema","newjeans neverdie","🔥 Trending Now","Hidden Gems","hanpham","🌴 Tropical Night","siu anh hùng"].randomElement() ?? fakeRooms[idx].roomName
                    fakeRooms[idx].movieTitle = ["Barbie","The Batman","Spider-Man","Joker","Inception","Tenet","Dunkirk","Memento","La La Land","Whiplash","Get Out","Us"].randomElement() ?? fakeRooms[idx].movieTitle
    ...
    var lobbyView: some View {
        VStack(spacing: 0) {
            ...
            ScrollView { VStack(spacing: 12) { ForEach(fakeRooms) { room in fakeRoomCard(room) } }.padding(.horizontal, 16).padding(.bottom, 120) }
        }
    }
    ...
    func fakeRoomCard(_ room: FakeRoom) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 8) {
                    HStack(spacing: -8) { ForEach(room.avatars.prefix(4), id: \.self) { av in Text(av).font(.system(size: 10)).frame(width: 20, height: 20).background(Circle().fill(Material.ultraThinMaterial.opacity(0.6))) } }
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                Text(room.currentTime).font(.system(size: 10, design: .monospaced)).foregroundColor(.white.opacity(0.7))
            }
    ...
    var createRoomView: some View {
        VStack(spacing: 24) {
            HStack {
                Button { showCreateRoom = false } label: { Image(systemName: "chevron.left").font(.system(size: 16, weight: .semibold)).foregroundColor(.white).padding(12).background(Circle().fill(Material.ultraThinMaterial.opacity(0.4))) }
    ...
    var inRoomView: some View {
        ...
        .onAppear {
            player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.5, preferredTimescale: 600), queue: .main) { t in
                let newTime = t.seconds; if newTime.isFinite { currentTime = newTime }
        ...
        VStack {
            if showControls {
                HStack {
                    ...
                    Spacer()
                    HStack(spacing: 4) {
                        Button { showEpisodePanel = true } label: { Image(systemName: "list.bullet").font(.system(size: 10)).foregroundColor(.white).padding(5).background(Circle().fill(.ultraThinMaterial.opacity(0.5))) }
                        Button { pipController?.startPictureInPicture() } label: { Image(systemName: "pip.enter").font(.system(size: 10)).foregroundColor(.white).padding(5).background(Circle().fill(.ultraThinMaterial.opacity(0.5))) }
                        Button { toggleOrientation() } label: { Image(systemName: "rotate.right").font(.system(size: 10)).foregroundColor(.white).padding(5).background(Circle().fill(.ultraThinMaterial.opacity(0.5))) }
                    }
                }.padding(.horizontal, 12).padding(.top, 8)
                Spacer()
                HStack(spacing: 44) {
                    Button { seek(-10) } label: { Image(systemName: "gobackward.10").font(.system(size: 22)).foregroundColor(.white) }
                    Button { player.rate == 0 ? player.play() : player.pause() } label: { Image(systemName: player.rate == 0 ? "play.fill" : "pause.fill").font(.system(size: 28)).foregroundColor(.white) }
                    Button { seek(10) } label: { Image(systemName: "goforward.10").font(.system(size: 22)).foregroundColor(.white) }
                }.padding(.bottom, 12)
            }
        }
    }
    ...
    var imessageChatPanel: some View {
                ...
                VStack(spacing: 2) {
                    ...
                    GeometryReader { g in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule().fill(.white.opacity(0.6)).frame(width: max(3, g.size.width * CGFloat(min(max(currentTime / max(duration, 1), 0), 1))), height: 3)
                        }
                    }.frame(height: 3)
        ...
        .background(ZStack {
            if let img = posterImage {
                Image(uiImage: img).resizable().aspectRatio(contentMode: .fill).blur(radius: 80).opacity(0.25)
            }
    ...
    func loadMovieForRoom(_ movie: Movie) {
        ...
        if let posterURL = movie.posterURL {
            Task { if let (data, _) = try? await URLSession.shared.data(from: posterURL), let img = UIImage(data: data) { await MainActor.run { posterImage = img } } }
        }
        
        Task {
            let urlString = "https://api.themoviedb.org/3/tv/\(movie.id)?api_key=b6be36c1c5788565fec6a24811e7cc9b&language=en-US"
            guard let url = URL(string: urlString) else { await MainActor.run { isLoadingSeasons = false }; return }
            do {
                ...
                if let fetched = try? await APIService.shared.fetchTVSeasons(tvId: movie.id) {
                    await MainActor.run { self.seasons = fetched; self.isLoadingSeasons = false }
                } else { await MainActor.run { self.isLoadingSeasons = false; self.seasonError = error.localizedDescription } }
            }
        }
    ...
    var episodePopupOverlay: some View {
        ZStack {
                    ...
                    ScrollView {
                        VStack(spacing: 8) {
                            ForEach(seasons) { season in
                                Button {
                                    withAnimation { selectedSeason = season; isLoadingEpisode = true
                                        Task { do { let detail = try await APIService.shared.fetchSeasonDetail(tvId: currentMovie?.id ?? 0, seasonNumber: season.seasonNumber); await MainActor.run { episodes = detail.episodes; isLoadingEpisode = false } } catch { await MainActor.run { isLoadingEpisode = false } } }
                                    }
                                } label: {
            ...
            .frame(width: 320)
            .background(RoundedRectangle(cornerRadius: 24).fill(.ultraThinMaterial).overlay(RoundedRectangle(cornerRadius: 24).stroke(.white.opacity(0.15), lineWidth: 0.5)))
    ...
    var viewerPanel: some View {
        VStack(spacing: 0) {
            ...
            ScrollView { VStack(spacing: 12) { ForEach(service.participants, id: \.userId) { p in
                HStack(spacing: 12) {
